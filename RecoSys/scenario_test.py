...
def recsys_top_n(df, up_num=100, top_num=3):
    # 카테고리 리스트 TOP20
    ...
    top_categories = df['categories'].value_counts().nlargest(20).index
    df['categories'] = df['categories'].apply(lambda x: x if x in top_categories else ['etc'])
     # 'categories' 컬럼의 값을 리스트에서 문자열로 변환
    df['categories'] = df['categories'].astype(str)
    
    num_filter = df.loc[df['book_count'] > up_num]
...
def merge_to_original_ReLearning(original_df, new_DF, length_new_df):
    if length_new_df >= 8: 
        new_DF_for_merge = review_sentiment_make_total(new_DF, length_new_df)
...
def review_sentiment_make_total(new_DF_for_merge, length_new_df):  
    # For backup save
...
if __name__ == '__main__':
    ...
    top_num_df = recsys_top_n(books, 100, 3)
    # 데이터 추가 및 재학습
    # new_DF_for_merge, length_new_DF_for_merge = adding_new_book(User_id) 
    # merge_to_original_ReLearning(original_df=ratings, new_DF=new_DF_for_merge, length_new_df=length_new_DF_for_merge)
